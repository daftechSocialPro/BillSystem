...
import { IMeterClassGetDto } from 'src/models/system-control/IMeterClassDto';
...
export class MeterClassComponent  implements OnInit {
  ...
  MeterClasses:IMeterClassGetDto[]
  ngOnInit(): void {
  ...
  getMeterClasses(){
    ...
    this.controlService.getMeterClass().subscribe({
      next:(res)=>{
  ...
  removeMeterClass(MeterClassId:string) {
    ...
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.controlService.deleteMeterClass(MeterClassId).subscribe({
          next: (res) => {
  ...
  updateMeterClass(MeterClass:IMeterClassGetDto){
