...
import { IMeterTypeGetDto } from 'src/models/system-control/IMeterTypeDto';
...
export class MeterTypeComponent  implements OnInit {
  ...
  getMeterTypes(){
    ...
    this.controlService.getMeterType().subscribe({
      next:(res)=>{
        this.MeterTypes = res 
      }
    })
  ...
  removeMeterType(MeterTypeId:string) {
    ...
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.controlService.deleteMeterType(MeterTypeId).subscribe({
          next: (res) => {
  ...
  updateMeterType(MeterType:IMeterTypeGetDto){
