...
import { IBillCycleGetDto } from 'src/models/system-control/IBillCycleDto';
...
export class BillCycleComponent implements OnInit {
  ...
  getBillCycles(){
    ...
    this.controlService.getBillCycle().subscribe({
      next:(res)=>{
        this.BillCycles = res 
      }
    })
  ...
  removeBillCycle(BillCycleId:string) {
    ...
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.controlService.deleteBillCycle(BillCycleId).subscribe({
          next: (res) => {
  ...
  updateBillCycle(BillCycle:IBillCycleGetDto){
